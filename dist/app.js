...
const generateQuestion = async (topic, answer) => {
    // Normalize answer
    ...
    const text = Object.values(response.query.pages)[0].extract;
    if (!text) {
    ...
    if (/is a (phylum|class|order|family|genus|species)/.test(text.slice(0, 100))) {
    ...
    const sentences = text
        ...
        .replace(/\b(Gallery|External links|Notes|References|See also|Selected works|Works)\b(.|\n)*/gm, "")
        ...
        for (const answer of alternates) {
            switch (topic) {
                case "Geography": {
                    result = result.replace(new RegExp(`\\b(the )?${answer}\\b`, "gi"), "<em>this place</em>");
                    break;
                }
                case "People": {
                    result = result.replace(new RegExp(`\\b${answer}\\b`, "gi"), "<em>this person</em>");
                    break;
                }
                default: {
                    if (answer.endsWith("s") &&
                        ...
                        (() => {
                            ...
                            return new RegExp(`\\b(${lastWord.slice(0, -2)}|${lastWord.slice(0, -1)})\\b`, "i").test(text);
                        })()) {
                        // Answer is plural. TODO: Handle -ies answers
                        result = result.replace(new RegExp(`\\b(the )?${answer}\\b`, "gi"), "<em>these</em>");
                    }
                    else {
                        if (answer.endsWith("y")) {
                            // Answer is singular -y
                            result = result.replace(new RegExp(`\\b(the )?${answer.slice(0, -1)}ies\\b`, "gi"), "<em>these</em>");
                        }
                        else {
                            // Answer is singular but not -y
                            result = result.replace(new RegExp(`\\b((an?|the) )?${answer}s\\b`, "gi"), "<em>these</em>");
                        }
                        result = result.replace(new RegExp(`\\b((an?|the) )?${answer}\\b`, "gi"), "<em>this</em>");
                    }
                    break;
                }
            }
        ...
        sentence.length > 40 &&
            ...
            !alternates.some(a => sentence
                .toLowerCase()
                .includes(a.toLowerCase().replace(/^the /i, "").slice(0, 4))) &&
            // Ignore orphaned references and comparisons
            !/(^(so|though|both|\w+ contrast|overall|then|he|she|they|these|those|that|this|it|later|such|this section|now|for example)|(subsequently|same|another|furthermore|similarly|such|therefore|however|thus))\b(?!<)/i.test(sentence));
    });
    ...
    let revealer = `${sentences[0].replace(/>[^<>]*( or .+?|, (also|commonly|known as|officially|sometimes|spelled) .+?,)(?= (is|are|was|were) )/, ">")}`;
    if (topic === "People") {
    ...
    return `<strong>${topic}</strong><br><br>${hints
        ...
        .join(" ")
        .replace(/\n/g, "<br>")}<br><br><strong>Answer:</strong> <a href="https://en.wikipedia.org/wiki/${encodeURIComponent(rawAnswer.replace(/ /g, "_"))}" target="_blank">${answer}</a>`;
};
let DEBUG_ANSWER;
...
(async () => {
    ...
    const answers = await (await fetch("vital-articles.json")).json();
    ...
    const loadQuestions = async () => {
        ...
        $questions.innerHTML = "";
        const topicsAndAnswers = Object.entries(answers[parseInt($difficulty.value) - 1])
        ...
        for (let i = 0; i < (DEBUG_ANSWER ? 1 : count); i++) {
            for (let j = 0; j < (DEBUG_ANSWER ? 1 : 5); j++) {
                try {
                    ...
                    const [topic, answer] = DEBUG_ANSWER !== null && DEBUG_ANSWER !== void 0 ? DEBUG_ANSWER : randomElement(topicsAndAnswers);
